...
\texttt{TÄHÄN ISTRUMENTAATIOSTA REFWORKS 24}
\begin{figure}[H]
  	\centering
...
The final stage of the process is data analysis, which can be done with a few different approaches \cite{RefWorks:25}:
\begin{itemize}
...
\label{sec:analysis}
 In order to gain beneficial usability information from user interface events, the log data has to be analyzed thoroughly. According to  Hilbert and Redmiles \cite{RefWorks:25}, there are several approaches to sort out important usability information out of log data.
...
\textbf{\emph{Detecting, comparing and characterizing sequences}} are approaches which utilizes sequence information of events. Sequence detection refers to action in which concretely or abstractly defined sequences are tried to be identified from the set of source sequences.
