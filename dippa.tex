...
\newcommand{\SUBTITLE}{The usability perspective}
\newcommand{\FTITLE}{Yrityksen sisäinen ohjelmistokehitysprosessi:}
\newcommand{\FSUBTITLE}{Käytettävyysnäkökulma}
...
Contextual Inquiry was originally developed to overcome three following challenges \cite{RefWorks:14}: 
\begin{enumerate}
...
Users' contributes to the design by providing a deep understading about the nature of the work. This is done through inquiry and it's a basis for fundamental work concepts.  As soon as the understanding about the work is available design for a system model can be created. \cite{RefWorks:14}
...
\label{sec:measurement}
	...
	Interaction Sequence Illustration uses data acquired during the contextual inquiry and doesn't gather any of its own. However, the inquiry data has to be complemented with documentation about interaction activities, for example by capturing the screen.
	There are three objectives for ISI methdod to handle. The first is to demonstrate how the user perceives the system. The second one is to identify and document activities, and to discover problems. The first two objectives creates the third one, which is to support 			the user-centred design and development. \cite{RefWorks:17}
	
	The strength of ISI lies in the analysis of data. 
