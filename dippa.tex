...
The subscriber of this thesis is a middle-sized company which is offering information services globally. The company is using in-house software development in order to create dynamic and bespoke software solutions and thereby aiming for commercial efficiency. It is not using any specific software development methodology, but operates in iterative manner. Because of the fast pace of growth, the company is willing to reform their current ERP system as well as the whole software development process. The reasoning for the new ERP system is the need to automate processes and to stop the increasing amount of new recruitments. The company aims to enhance its performance by fifteen percent with the new system. This thesis aims to give answers to the following research questions:
...
The analyst should consider the following few questions while going trough the task sequence or different stages on it. The questions should be used as a guidelines or criteria for creating a credible story about the interaction between the user and the system. \cite{RefWorks:26}
\begin{itemize}
...
The first question should be asked to make sure that the user would understand what need to be done to proceed. The second question on the other hand tries to ensure that the user can notice all the relevant actions which need to be done are available.
The third question should be asked to perceive poor naming of the actions. The last question need to be considered to ensure that the necessary feedback is provided for the user. 

The story created in the analysis should include credible success stories and failure stories. The success stories are being created when the system works and the user could credibly proceed from one task stage to another. If the credible story cannot be created and the story fails under the criteria, failure story should be included in the analysis and it should provide a description why the user would face the problem. Using stories the system's advantages and disadvantages can be described. \cite{RefWorks:26} 		
...
The researcher must have a profound understanding about the context of use, in order to \emph{specify the user requirements}. A qualified system can be implemented only if the user requirements are precisely defined. Many of the methods used in this experiment aims to assist in this crucial state of development. Technically, the process of requirement specification can be initiated while running the Contextual Inquiry or Remote Contextual Inquiry. Although, it is essential to remember that the focus in CI and RCI should be (at the first stages of the process) mostly on understanding the context of use. This thesis uses user evaluation, remote usability evaluation, expert evaluation and quantitative metrics to identify all the user requirements affecting on the user experience and the functionality of the system. 
...
The user experience might have influence on the efficiency of use. This is why the UX should be considered also in the development of distinctive ERP system for experts. The System Usability Scale is being used to understand the level of user satisfaction, which is highly connected, among many other factors (see section \ref{sec:principles}), with the user experience.

Traditional and continuous usability evaluation would be difficult and expensive for the subscriber company because of its distributed operational environment. Hence, the possibility of accommodating remote usability evaluation to the company's software development process is examined in this thesis. Remote usability evaluation in the context of this thesis stands for the remote usability related user interface data logging and Remote Contextual Inquiry. The desirable results from the data logging and RCI would be utilizable, localized usability data, which could help the development of the global ERP system and to reveal cultural dependent user requirements. 
...
After the user requirements have been specified, \emph{design solution can be produced according to these requirements}. Analyzed data is then applied to create a new user-centered prototype solution. The solution and \emph{the design is then evaluated against the requirements}. Mockups are used to evaluate the design solutions before the implementation of the high-fidelity prototype.

Figure \ref{fig:hcd_process} illustrates the methods used and the actions taken in the applied design process and reflects them against the actual human-centered design process. Contextual Inquiry, Interaction Sequence Illustration and System Usability Scale are utilized to understand the context of use and the level of sophistication of the current solution. Those methods, in addition to Cognitive Walkthrough, are applied to specify the user requirements. Then the SUS and ISI methods measures the existing solution and the data is exploited in analysis and user requirement specification. The design solutions and the prototype is evaluated with Cognitive Walkthrough, ISI, CI, RCI, remote usability data logging and System Usability Scale. The final analysis is carried out using the data from the second evaluation.  

The applied human-centered design activities described in this section could be attach to iterative software development quite easily. In practice, the following iterations would create only few changes in the applied process and in the methods. Contextual Inquiry and Remote Contextual Inquiry could be used to identify the issues which needs to be improved in the following iterations. System Usability Scale could be used to gather data about the improvements in order to understand their consequences. The SUS could probably be even more useful in the following iterations, since there would be earlier scores available for comparison. Interaction Sequence Illustration could give beneficial information during the first iteration about the quantitative changes in interaction stages and steps and about the complexity of the interaction. In the long run it would probably get ineffective if the iterative updates would be small and no changes would be made in the interaction sequence itself. Although, if the software development iteratively increments the number of processes attached to the system, then ISI can be used in every iteration to assess the difference between the old and new implementation of the process. Remote usability logging could be used during the iterations to gather quantitative information and therefore assessments about the success rate of the iteration. Cognitive Walkthrough could be used as an anticipatory test for the iteration before the update is published. Furthermore, the focus of the methods can vary also inside an iteration. For example the second stage of the applied design process utilizes Contextual Inquiry to understand the context of use whereas the last stage uses CI to evaluate the design against the requirements. 
...
\subsection{Prototype and remote usability logging library creation}

The prototype was created based on the findings of the first evaluation round and it was implemented with the Microsoft's latest .NET 4.5 software framework. Because of the organizational requirements the prototype was decided create as a desktop application without any real back-end solution. The user interface was created with Windows Presentation Foundation (WPF) and C!!!!!!!!!!!!!!!!!!!!!!!!!!, utilizing the Model View ViewModel (MVVM) presentation pattern in order to achieve Separation of Concerns (SoC). \cite{RefWorks:37} Also, a third party components were used in the implementation in order to create the prototype more efficiently. 

In order to utilize remote usability logging as a usability evaluation method, a few solutions were reviewed. All the third party solutions were not seen as cost efficient, so the library for the remote usability logging was implemented as a part of the thesis. The library consists of a singleton pattern and it's static methods on top of the Windows Communication Foundation (WCF) service. The library supplies methods for logging sessions and events during the user interface interaction and saves the data, including serialized Extensible Markup Language (XML) data of the arbitrary object to the database. The prototype references the library and the user interface code is decorated with the remote logging functionality. The library gathers information such as time --------------------------

	  
	  \subsection{Interaction Sequence Illustration}
=======
>>>>>>> 59c8292b80458ae20ce4d7585a6942794e9c280d

   
The practical implementation of the research study took place between August and DECEMBER???? 2013. It covers the first iteration of the applied human-centered design and utilizes all the stages described in the previous section (see section \ref{sec:dprocess}). 

Altogether seven users from two different countries contributed in the study by participating the usability evaluation sessions. The only usability inspection method, the Cognitive Walkthrough, was utilized by one analyst. The usability evaluation methods used were remote data logging, Contextual Inquiry, Remote Contextual Inquiry and Interaction Sequence Illustration evaluations. The participants also filled in the System Usability Scale form. The users' work experience varied from under a year to twenty five years. In every evaluation session the users were situated in their own workstations. 

The chosen business process for the research study was the result of a long consideration. Due to limitations of the schedule, the process had to be restricted. Also, it had to be notable part of the system, with the option to track things such as average processing time and number of interaction steps. Finding an applicable segment proved to be a challenging task, partly because of the tailored nature of the ERP system. Finally, the customer management process was chosen because it was seen as a suitable part of the system to be studied and it met the requirements for the thesis quite well. The customer management process also had some anticipated usability issues.  

The study was implemented in two phases and in one applied human-centered design iteration. The aim of the first evaluation round was to gain understanding about the current system, including its deficiencies and assets. The focus was set for understanding the process and the way user's are utilizing the system in their everyday work. The objective of the second iteration on the other hand was to evaluate and redefine the requirements of the new system (gathered with the usability methods). In practice, this was accomplished by using the usability methods to evaluate the created prototype. For understandability this section is divided into two subsections according to the two phases of the practical study. However, the reader should keep in mind that the two phases adapts all the stages of the applied human-centered design process.

\subsection{Phase 1: Evaluating the existing system}
\label{sec:phase1}
The first phase encompassed the Contextual Inquiry, Remote Contextual Inquiry, System Usability Scale, Interaction Sequence Illustration and Cognitive Walkthrough. Firstly, Contextual Inquiry was utilized together with the ISI and SUS methods in Finland. Overall four users participated in the inquiry and the had working experience from half a year to 21 years in the company. In average, the working experience in the company was approximately nine years. All of the users had previous customer service experience out of the company. The aim of this stage was to gain understanding about the context of use, test the methods in practice and specify the user requirements. 

A desirable situation would have been the one where the ISI could have been used to measure the average time of the customer creation process and that the Contextual Inquiry would have been targeted on a real customer creation situation. During the first inquiry, it cleared that the users don't have enough real-life cases to be used in Contextual Inquiry and Interaction Sequence Illustration. Also, the practical implementation of the process turned out to be changing according to the customers' needs, provided information and the customer service representative's conventions, so it came clear that measuring the time of completing the process was not a good idea. Because the same reason, utilizing the ISI became more difficult. To make the implementation of Interaction Sequence Illustration beneficial, a set of tasks were created according to the assumed customer creation process as whole and the users were asked to accomplish those tasks. The activities were recorded and the users were asked to point out all the problems in the predefined activity set. This way it was possible to refine the stages and the interaction steps of the conventional customer creation process. 

During the interviews the customer service representatives had only some parts of the customer creation process in their to-do lists. Because the lack of real-life cases, the users were working with the customer management artificially during the inquiries. This was enabled by using the test environment of the system. The authentic parts of the inquiries were naturally implemented in the production environment. The inquiries and ISI activities, which took place in Finland, were implemented in the users' workstations. The materials gathered from the inquiries were audio records and ISI activities were video recorded. Also, notes were written during the meetings. After the meetings, a Finnish version of the SUS questionnaire was sent to all of the participants as a web form.  

Secondly, Remote Contextual Inquiry, Interaction Sequence Illustration and System Usability Scale were implemented with three Swedish users. Users' working experience in the company varied between 0.5 years to two years, but as well as with the Finnish users, all of them had earlier experience from the field of customer service. Average years of the Swedish users in the company were 1.33 years. Inquiries and ISI activities were implemented basically in a same way than in Finland. Inquiries were made about the real-life cases if there were any in progress, but most of the time the test environment of the system were utilized. The ISI was implemented with the same predefined set of activities as in Finland and then refined with the users during the meeting. The aim of this stage of the study was to understand and specify the context of use, specify the user requirements and to examine if there are any differences between the two country offices and the way they interact with the system.

Remote Contextual Inquiry were implemented with the Swedish users instead of Contextual inquiry because of the different geographical locations of the users and the thesis worker. The tool chosen for the remote inquiries was a web conferencing software WebEx, which provided all the necessary needed functions, such as screen sharing, telephone conference, audio recording and video recording. The materials gathered from the interviews were then screen capture video with the audio recorded discussions and handwritten notes. The method related challenges faced with the CI in Finland weren't valid anymore with the Swedish users, but RCI caused some surprising technical problems, such as problems with the microphones and some authentication difficulties. After the meetings, an English version of the SUS questionnaire was sent to all of the participants as a web form. 

Every evaluation session with the Finnish and Swedish users went on for a bit less than two hours, so altogether the user evaluations of this phase took approximately fourteen hours. The response rate of the SUS questionnaire after the first phase was hundred percent. The know-how level of the process and the system was varying considerably between the users. However, all of the participants were using the system and implementing some parts of the customer creation process weekly or daily.

The first usability inspection was implemented after the user evaluations with the Cognitive Walktrough. It was carried out by the thesis worker in order to assure that the level of recognized major issues of the system was as high as possible. The inspection was executed in the system's test environment and according to the instructions of the method. The results were gathered by reviewing the guideline questions of the method and by documenting all the success and failure stories. 

 

 
\subsection{Phase 2: Building and evaluating the high-fidelity prototype}

The first task to accomplish after the first phase (see subsection \ref{sec:phase1}) wast to design the new user interface solution for the process and create mockups. The mockups were build by using the storyboarding tool of Microsoft's Visual Studio Ultimate 2012 and evaluated with three customer service specialists in Finland. After evaluating and refining the design in a multiple evaluation meetings, the software development was ready to start.

%
%
%	  \subsection{Cognitive Walkthrough}
%	  -toteutin yksin, noudatin ohjeistusta, tein tehtävän testiversiolla
%	  \\*
%	  -kysyin annettuja kysymyksiä itseltäni
%	  \\*
%	  -kirjasin ylös success storyt ja failuret.
%	  \\*
%	  -2.vaihe, testaus ennen prototyypin julkaisua.
%	  \\*
%	  
...
%    	 \\*
%    	 -vuodet yrityksessä (noin): 1,21,14,0.5 = avg.9 suomessa
%    	 \\*
%    	 -suomessa työympäristö
%    	 \\*
%    	 -ruotsista 3 osallistujaa
%    	 \\*
%    	 -vuodet yrityksessä (noin): 2,0.5,1.5 = avg. 1.33 ruotsissa
...
%	  \\*
%	   ymmärrys järjestelmästä ja prosessista. käyttäneet paljon, päivittäin/viikoittain.
%	  \\*
%	  -ruotsiin tehtiin rci:llä 
%	  \\*
...
%	  \\*
%	  -pyrittiin mahdollisimman laaja-alaiseen täyttöön esimerkki-tehtävässä, mutta annettiin vapaat kädet.. testikannassa toteutettiin koko hierarkian luominen.
%	  \\*
%	  -analyysi toteutettiin menetelmän ohjeiden mukaisesti sillä erotuksella että järjestelmästä laskettiin keskimääräiset kentät.
...
-toteutettiin myös kahdessa osassa. ennen ja jälkeen prototyypin. 
-CI affiniteettidiagrammilla.
-CW normaalilla tavalla
-SUS normaalilla tavalla.
-ISI
     	  -interaktioiden ajallinen mittaaminen ei onnistunut
...
\\
    ...
    \\*
     ...
     \\*
	vaihe1: ruotsissa tulos: 49.1667/100
	\\*
...
-tutkimus kootaan yhteen ja vedetään päätelmät tuloksiin perustuen. arvioidaan tutkimuksen onnistumista.
